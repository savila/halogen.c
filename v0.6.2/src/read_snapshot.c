 ...
 *                                PROTOTYPES
 *=============================================================================*/
void read_gadget(FILE *icfile,float ***out_x,float ***out_y, float ***out_z,float ***out_vx,float ***out_vy, float ***out_vz,int ifile);
long get_pid(int i);
 ...
 *                                   MAIN
 *=============================================================================*/
int read_snapshot(char *infile_name, int format, float lunit, float munit, int swp, int glong, int gdouble, float ***out_x, float ***out_y, float ***out_z, float ***out_vx, float ***out_vy, float ***out_vz,long *out_Np, float *out_mp, float *out_L, float *out_omega_0, int *out_Nfiles){
    ...
    while((icfile = fopen(gadget_file,"rb")) != NULL)
     {
      ...
      (* out_Nfiles) = no_gadget_files;
      if(!((*out_x)=(float **) calloc(no_gadget_files, sizeof(float *))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((*out_y)=(float **) calloc(no_gadget_files, sizeof(float *))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((*out_z)=(float **) calloc(no_gadget_files, sizeof(float *))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((*out_vx)=(float **) calloc(no_gadget_files, sizeof(float *))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((*out_vy)=(float **) calloc(no_gadget_files, sizeof(float *))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((*out_vz)=(float **) calloc(no_gadget_files, sizeof(float *))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
    ...
    if(no_gadget_files > 1)
     {
      ...
      #pragma omp parallel for private(i_gadget_file,icfile,gadget_file) shared(no_gadget_files,gadget,infile_name,stderr,out_x,out_y,out_z,out_vx,out_vy,out_vz) default(none)
      for(i_gadget_file=0; i_gadget_file<no_gadget_files; i_gadget_file++)
       {
        ...
        /* read files... */
        read_gadget(icfile,out_x,out_y,out_z,out_vx,out_vy,out_vz,i_gadget_file);
        fclose(icfile);
       } 
   ...
   *===============================*/
  else
   {
    ...
    read_gadget(icfile,out_x,out_y,out_z,out_vx,out_vy,out_vz,0);
    fclose(icfile);
  ...
  for(i=0;i<no_part;i++)
   {
    ...
    (*out_vx)[ifile][i] = ddummy[0];
    (*out_vy)[ifile][i] = ddummy[1];
    (*out_vz)[ifile][i] = ddummy[2];
   }
  /*================= read in GADGET velocities =================*/
