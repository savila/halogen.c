...
//Actually placing the haloes----------------------------------- 
	...
	for (ihalo=0;ihalo<Nend;ihalo++){
		...
		if(ihalo>0){
		if(1.0-pow(1.0-pow(Mcell/mpart,exponent)/TotProb,ihalo-n_recalc+20)>recalc_frac){
			recalc = 1;
			n_recalc += 1;
			fprintf(stderr,"RECALCULATING: %ld, %ld, %e, %e\n",n_recalc,ihalo,1.0-pow(1.0-pow(Mcell/mpart,exponent)/TotProb,ihalo-n_recalc+20),TotProb);
		}
		}
