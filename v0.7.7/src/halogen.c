...
int main(int argc, char **argv){
	...
	fprintf(stderr,"\n");
	if (place_halos(Nhalos,HaloMass, Nlin, Npart, x, y, z, vx,vy,vz,Lbox, rho,seed,mpart, nthreads,alpha, betavec, Malpha, Nalpha,recalc_frac,hx, hy, hz, hvx,hvy,hvz, hR,ListOfParticles,NPartPerCell,dens)==0){
...
int write_halogen_cat(char *filename, float *x, float *y, float *z, float *vx, float *vy, float *vz, float *M, float *R,long N){
	...
	for(i=0;i<N;i++){
		fprintf(f,"%f %f %f %f %f %f %e %f\n",x[i],y[i],z[i],(vx[i]*VEL_FACT),(vy[i]*VEL_FACT),(vz[i]*VEL_FACT),M[i],R[i]);
	}
	fclose(f);
	fprintf(stderr,"Used VEL_FACT=%f\n",VEL_FACT);
	return 0;
}
