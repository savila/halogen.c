...
int read_snapshot_serial(char *infile_name, int format, float lunit, float munit, int swp, int glong, int gdouble, int NCells, int nthreads, float **out_x, float **out_y, float **out_z, float **out_vx, float **out_vy, float **out_vz,long *out_Np, float *out_mp, float *out_L, float *out_omega_0,long ***ListOfPart,long **NPartPerCell,float **dens){
  char    gadget_file[MAXSTRING];
  int     no_gadget_files, i_gadget_file;
  long   i,j,k,lin_ijk;
  FILE   *icfile;
   ...
   *===============================*/
  else
   {
	...
	//Alloccing memory for particle
 	...
 	t3=time(NULL);
	diff=difftime(t3,t2);
      	fprintf(stderr,"\nSER: Time in intermediate steps: %f\n\n",diff);

 	
	(MassLeft) =  pos_2_cic_serial(NCells,(*out_x),(*out_y),(*out_z),(*ListOfPart),(*NPartPerCell));
	(*dens) = (float *) interp_dens(NCells,(*out_x),(*out_y),(*out_z),(MassLeft));
...
double *pos_2_cic_serial(int n_grid,float *X,float *Y, float *Z,long **ListOfPart,long *NPartPerCell)
{
  //////
  // Returns a grid[n_grid*n_grid*n_grid] with    
  // the density field calculated using the CIC
  ...
  count = (long *) calloc(NCELLS*NCELLS*NCELLS,sizeof(long));
  if(count == NULL) {
        fprintf(stderr,"\tplace_halos(): could not allocate %d array for PerCell[]\nABORTING",NCELLS*NCELLS*NCELLS);
        exit(-1);
  }
...
#pragma omp parallel num_threads(NTHREADS) default(none)      \
  shared(X,Y,Z,n_grid,np,grid,l_box,mpart,invL,NCELLS,stderr,count,ListOfPart)
  {
    int ii;
    long count_i;
    long i,j,k,lin_ijk;
...
#pragma omp for  nowait
    for(ii=0;ii<np;ii++) {
      ...
      k = (long) (invL * zp);
      if (i<0 || i>=NCELLS || j<0 || j>=NCELLS || k<0 || k>=NCELLS){
                        fprintf(stderr,"\tERROR: Particle %d at [%f,%f,%f] seems to be out of the right box interval [0.,L)",ii,xp,yp,zp);
      }
      lin_ijk = k+j*NCELLS+i*NCELLS*NCELLS;

//#pragma omp atomic
      count_i = count[lin_ijk]; 
#pragma omp atomic
      count[lin_ijk]++;

      ListOfPart[lin_ijk][count_i] = ii;
