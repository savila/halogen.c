...
//Initiallising -------------------------------------------------
	...
	if (L/NCells<R_from_mass(HaloMass[0],rho_ref)){
		fprintf(stderr,"WARNING: cell size is smaller than the radius of the biggest halo. Using r=%f. This may be problematic\n",r);
		//exit(0);
	}
...
//				fprintf(stderr,"\thalo: [[%f,%f,%f],[%f,%f,%f],%f]\n",HaloX[ihalo],HaloY[ihalo],HaloZ[ihalo],HaloVX[ihalo],HaloVY[ihalo],HaloVZ[ihalo],HaloR[ihalo]);
			#ifdef NO_EXCLUSION
				#ifdef BETA0
				fprintf(stderr,"WARNING: NO_EXCLUSION and BETA0 are not compatibles!\n");
				#else
				if(copyof_dens[ipart] == 0.)
			  		check = 1;
				else
...
//#endif
  ...
  if (i>=0 && i<NCells && j>=0 && j<NCells && k>=0 && k<NCells){
	...
	fprintf(stderr,"WARNING: Computing distances outside the box. Cell: [%d,%d,%d].\n",i,j,k);
	return 0;
  }
}
