...
*/
  ...
  if (ThisTask == 0){
      ...
      if(!((file_y)=(float **) calloc(NTask, sizeof(float *))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_z)=(float **) calloc(NTask, sizeof(float *))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_vx)=(float **) calloc(NTask, sizeof(float *))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_vy)=(float **) calloc(NTask, sizeof(float *))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_vz)=(float **) calloc(NTask, sizeof(float *))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      ...
      if(!((file_y[0])=(float *) calloc(Nthisfile, sizeof(float))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_z[0])=(float *) calloc(Nthisfile, sizeof(float))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_vx[0])=(float *) calloc(Nthisfile, sizeof(float))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_vy[0])=(float *) calloc(Nthisfile, sizeof(float))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_vz[0])=(float *) calloc(Nthisfile, sizeof(float))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
	...
	//should be overcome
        ...
        file_y[0][i]=(float)P[i].Pos[1];
        file_z[0][i]=(float)P[i].Pos[2];
        file_vx[0][i]=(float)P[i].Vel[0];
        file_vy[0][i]=(float)P[i].Vel[1];
...
if (ThisTask!=0) {
  ...
  if(!((thisfile_y)=(float *) calloc(NumPart, sizeof(float ))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
  if(!((thisfile_z)=(float *) calloc(NumPart, sizeof(float ))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
  if(!((thisfile_vx)=(float *) calloc(NumPart, sizeof(float ))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
  if(!((thisfile_vy)=(float *) calloc(NumPart, sizeof(float ))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
  if(!((thisfile_vz)=(float *) calloc(NumPart, sizeof(float ))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
  for (i=0;i<NumPart;i++){	
	...
	thisfile_z[i]=(float)P[i].Pos[2];
	thisfile_vx[i]=(float)P[i].Vel[0];
  ...
  MPI_Send( &thisfile_z[0], Nthisfile, MPI_FLOAT, 0, 1113, MPI_COMM_WORLD);
...
//MPI_Barrier(MPI_COMM_WORLD);
  if (ThisTask == 0){
	...
	MPI_Recv( &thisfile_vz[0], Nthisfile, MPI_FLOAT, groupTask, 1116, MPI_COMM_WORLD, &status);
      ...
      if(!((file_y[groupTask])=(float *) calloc(Nthisfile, sizeof(float))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_z[groupTask])=(float *) calloc(Nthisfile, sizeof(float))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_vx[groupTask])=(float *) calloc(Nthisfile, sizeof(float))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_vy[groupTask])=(float *) calloc(Nthisfile, sizeof(float))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
      if(!((file_vz[groupTask])=(float *) calloc(Nthisfile, sizeof(float))))
      {
        fprintf(stderr,"\nfailed to allocate memory for GADGET data\n");
        exit(1);
      }
