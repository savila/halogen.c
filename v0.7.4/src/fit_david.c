...
int find_best_alpha(double *MassLeft, float *dens){
	...
	thisseed = malloc(Nalpha*num_alpha*ntrials*sizeof(long));
	seed = time(NULL);
	for (i=0;i<Nalpha;i++){
  ...
  shared(num_alpha,ntrials,i,alpha_grid,low,da,stderr,Nalpha,alphavec,iii,mcuts,dens,ListOfParticles, \
	 ...
	 total_nr,MassLeft,Nhalos) private(jk,j,k,RemMass) num_threads(nthreads)
#endif
		{
		double *RemMass;
   ...
   #pragma omp for  
		...
		for(jk=0;jk<num_alpha*ntrials;jk++){
			...
			place_halos(nend,HaloMass, Nlin, Npart, x, y, z, x,y,z,Lbox,
					...
					mpart,64,thisalphavec, betavec, mcuts, Nalpha, Nrecalc,hx, hy, hz,
