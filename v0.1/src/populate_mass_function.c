...
long populate_mass_function(char *filename, double Mmin, double Mmax, double Lbox, double **halo_masses){
	...
	fprintf(stderr,"...done!\n");
	*halo_masses = (double *) calloc(Nhalos,sizeof(double));
	...
	for (i=0;i<Nhalos;i++){
		...
		(*halo_masses)[i] = spline_inter(dens_inv_arr,mass_inv_arr,Nshort,y2,d_rand);
	}
