...
//Actually placing the haloes----------------------------------- 
	...
	for(i_alpha=0;i_alpha<Nalpha;i_alpha++){
		...
		fprintf(stderr,"\tNstart=%ld, Nend=%ld\n",Nstart,Nend);
                #pragma omp parallel for private(Mhalo,ihalo,Mcell,R,i,j,k,lin_ijk,check,trials,ipart,icell,rho_ref) shared(HaloMass,NCells,TotProb,Halos_done,Nend,Nstart,i_alpha,stderr,ListOfHalos,ListOfPart,NHalosPerCell,NPartPerCell,MassLeft,PartX,PartY,PartZ,PartVX,PartVY,PartVZ,HaloX,HaloY,HaloZ,HaloVX,HaloVY,HaloVZ,HaloR,CumulativeProb,stdout) default(none)
		for (ihalo=Nstart;ihalo<Nend;ihalo++){
			...
			do {	
				...
				if (ihalo==0)
		  		...
		  		do {
					if (ihalo==0)
						fprintf(stderr,"%ld d",ihalo);	
					ipart = select_part(lin_ijk,ListOfPart, NPartPerCell);		
					if (ihalo==0)
						fprintf(stderr,"%ld e",ihalo);	
					...
					if (ihalo==0)
						fprintf(stderr,"%ld f",ihalo);	
					R=R_from_mass(HaloMass[ihalo],rho_ref);
					HaloR[ihalo]= R;
					if (ihalo==0)
						fprintf(stderr,"%ld g",ihalo);	
					#ifdef NO_EXCLUSION
					check = 0;
					#else
