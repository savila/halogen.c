...
int main(int argc, char **argv){
	...
	char inname[256];
	long Npart, Nhalos, **ListOfParticles, *NPartPerCell,ihalo;
	int ialpha;
	float *HaloMass, rho;
	#ifdef VERB
...
#endif
#ifdef NO_PROB_PARALLEL
	fprintf(stderr,"#def NO_PROB_PARALLEL\n");
#endif
#ifdef NO_HALOS_PARALLEL
	fprintf(stderr,"#def NO_HALOS_PARALLEL\n");
#endif
#ifdef NO_READ_PARALLEL
	fprintf(stderr,"#def NO_READ_PARALLEL\n");
#endif
	...
	fprintf(stderr,"...Halo Masses Generated for %ld halos\n",Nhalos);
	#ifdef DEBUG
		fprintf(stderr,"\n M[%d]=%e\n",0,HaloMass[0]);
		fprintf(stderr,"\n M[%d]=%e\n",1,HaloMass[1]);
		fprintf(stderr,"\n M[%ld]=%e\n",Nhalos -1,HaloMass[Nhalos-1]);
	#endif
        t2=time(NULL);
        diff = difftime(t2,t1);
	fprintf(stderr,"\nTime Generating Masses: %f\n",diff);
	...
	for (ihalo=0;ihalo<Nhalos;ihalo++){
		if(HaloMass[ihalo]<Malpha[ialpha]){
			ialpha++;
			if (ialpha>0)
				fprintf(stderr,"N[%d]=%d\n",ialpha-1,Nhalosbin[ialpha-1]);
			fprintf(stderr,"\nalpha[%d]=%e   M[%d]=%e",ialpha,alpha[ialpha],ialpha,Malpha[ialpha]);
			if (ialpha==Nalpha){
	...
	fprintf(stderr,"N[%d]=%d\n",ialpha,Nhalosbin[ialpha]);
	for (ialpha=0; ialpha<Nalpha; ialpha++){
		fprintf(stderr,"\t%d\t%f\t%e\t%d\n",ialpha,alpha[ialpha],Malpha[ialpha],Nhalosbin[ialpha]);
	}
	//place the halos
	fprintf(stderr,"Placing halos down...\n");
	if (place_halos(Nhalos,HaloMass, Nlin, Npart, x, y, z, vx,vy,vz,Lbox,rho,seed,mpart, alpha, Nhalosbin, Nalpha, hx, hy, hz, hvx,hvy,hvz, hR,ListOfParticles,NPartPerCell)==0)
