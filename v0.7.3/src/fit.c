...
// ------------- GLOBAL PARAMETERS ---------------------------------------------
float *x, *y, *z, Lbox, mpart, *HaloMass, om_m, rho;
...
long read_nbody(float **nbx, float **nby, float **nbz, float **nbm){
	...
	//	#ifdef NO_PARALLEL_FIT	
		for(j=0;j<num_alpha;j++){
			...
			for(k=0;k<ntrials;k++){
				...
				//set_seed(iii);
				fprintf(stderr,"MCUTS %d: ", iii);
				for(ir=0;ir<Nalpha;ir++){
					fprintf(stderr,"%e, ",mcuts[ir]);
				}
				fprintf(stderr,"\n");
				// Place the halos
				...
				correlate(nend, Lbox,hx,hy,hz,trials_2pcf[k], ercorr, DD);
			...
			for (ir=0;ir<nr;ir++){
				alpha_2pcf[j][ir] = 0.0;
				//fprintf(stderr,"ir=%ld/%d\n",ir,total_nr);
				for(k=0;k<ntrials;k++){
					alpha_2pcf[j][ir] += trials_2pcf[k][ir+total_nr-nr];
				}
				alpha_2pcf[j][ir] = alpha_2pcf[j][ir]/ntrials; 
				//fprintf(stderr,"middle\n");
				alpha_err[j][ir] = 0.0;
				for(k=0;k<ntrials;k++){
					alpha_err[j][ir] += pow((trials_2pcf[k][ir+total_nr-nr]-alpha_2pcf[j][ir]),2);
				}
				alpha_err[j][ir] = pow((alpha_err[j][ir]/(ntrials-1)),0.5);
				//fprintf(stderr,"end\n");
				// Now get chi^2 values
				chi2 += pow(((alpha_2pcf[j][ir]-nbody_2pcf[i][ir])/alpha_err[j][ir]),2);
				//fprintf(stderr,"CHI2: %ld, %e, %e, %e, %e\n", ir, chi2, alpha_2pcf[j][ir], nbody_2pcf[i][ir],alpha_err[j][ir]);
			}
			gsl_vector_set(chi2_alpha,j,chi2);
			gsl_vector_set(weights,j,1./chi2);
		}
		//#endif
		...
		*/
		...
		NPartPerCell,x,y,z,Lbox,Npart,Nlin,HaloMass,nend,mpart,recalc_frac,ercorr,DD,betavec,thisseed,trials_2pcf,rho,maxr,\
		...
		for(jk=0;jk<num_alpha*ntrials;jk++){
			...
			RemMass = (double *)calloc(Nlin*Nlin*Nlin,sizeof(double));
			if (RemMass==NULL){
			...
			int itemp;
			for (itemp=0;itemp<Nlin*Nlin*Nlin;itemp++)
				RemMass[itemp] = MassLeft[itemp];
...
/*
	...
	*/
		...
		for (xi=gsl_vector_get(alpha_grid,0);xi<gsl_vector_get(alpha_grid,num_alpha-1);xi+=0.1){
			gsl_bspline_eval(xi, B, bw);
