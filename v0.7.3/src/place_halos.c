...
//	fprintf(stderr,"Mleft[0],Mleft[last]:  %e, %e\n",MassLeft[0],MassLeft[Nlin*Nlin*Nlin-1]);
	...
	fprintf(stderr,"FIRST UP RANDOM NUMBER: %ld",rand());
...
// ------------------------------------------------- Initiallised
	...
	fprintf(stderr,"\t bbb \n");
  	//Temporary for comparison
	for(i=0;i<10;i++){
	  fprintf(stderr,"MASSLEFT VECTOR %ld: %f",i,MassLeft[i]);
	}
	for(i=NTotCells-10;i<NTotCells;i++){
	  fprintf(stderr,"MASSLEFT VECTOR %ld: %f",i,MassLeft[i]);
	}
...
//				fprintf(stderr,"\thalo: [[%f,%f,%f],[%f,%f,%f],%f]\n",HaloX[ihalo],HaloY[ihalo],HaloZ[ihalo],HaloVX[ihalo],HaloVY[ihalo],HaloVZ[ihalo],HaloR[ihalo]);
		...
		//TEMPORARY FOR COMPARISON
		fprintf(stderr,"Iteration: %ld, Mcell: %e, lin_ijk: %ld, RAND: %ld\n",ihalo,Mcell,lin_ijk,rand());
