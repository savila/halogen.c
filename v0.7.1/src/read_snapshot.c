...
//        fprintf(stderr,"\n\t===================================================================\n");
   ...
   *===============================*/
  else
   {
	...
	(*dens) = interp_dens(NCells,(*out_x),(*out_y),(*out_z),(*MassLeft));
...
double *pos_2_cic(int n_grid,float *X,float *Y, float *Z)
{
  ...
  double *grid=(double *)calloc(n_grid*n_grid*n_grid,sizeof(double));
  if(grid==NULL) {
...
float *interp_dens(int n_grid,float *X,float *Y, float *Z,double *grid)
{
  ...
  int np = gadget.nall;
  double l_box=gadget.header.BoxSize;
  float *dens=(float *)calloc(np,sizeof(float));
  if(dens==NULL) {
