...
long select_part(long, long **,long *,float*);
int check_HaloR_in_mesh(long,float *, float *, float * , float *,long,long,long,long*,long*,long*);
...
//Initiallising -------------------------------------------------
	...
	long Nhalos;
	float *copyof_dens;
	int number_of_betas=0;
	#ifdef BETA_0
	fprintf(stderr,"#def BETA_0\n");
	number_of_betas++;
	#endif
	#ifdef BETA_1
	fprintf(stderr,"#def BETA_1\n");
	number_of_betas++;
	#endif
	#ifdef BETA_INF
	fprintf(stderr,"#def BETA_INF\n");
	number_of_betas++;
	#endif
	#ifdef BETA_FREE
	fprintf(stderr,"#def BETA_FREE = %f\n",BETA);
	number_of_betas++;
	#endif
	#ifdef NGP
	fprintf(stderr,"#def NGP\n");
	#endif
	if (number_of_betas!=1){
		fprintf(stderr,"ERROR: Need to have one and only one BETA defined (BETA_0,BETA_1,BETA_INF,BETA_FREE)\n");
		return -1;
	}
	...
	memcpy(MassLeft,RemMass,NTotCells*sizeof(double));
	#else
	MassLeft = (long *) calloc(NTotCells,sizeof(long));
  	if(MassLeft == NULL) {
	...
	copyof_dens = malloc(NTotPart*sizeof(float));
	memcpy(copyof_dens,dens,NTotPart*sizeof(float));
...
//		NPartPerCell[lin_ijk]++;
	...
	for (k=0;k<NCells;k++){
		...
		Nhalos += (long) (NPartPerCell[lin_ijk]/Nmin+1);
		#ifdef NGP
...
//			fprintf(stderr," = [%ld,%ld,%ld]\n",i,j,k);
		  ...
		  do {
			ipart = select_part(lin_ijk,ListOfPart, NPartPerCell,copyof_dens);		
			if (ipart<0){
...
//				fprintf(stderr,"\thalo: [[%f,%f,%f],[%f,%f,%f],%f]\n",HaloX[ihalo],HaloY[ihalo],HaloZ[ihalo],HaloVX[ihalo],HaloVY[ihalo],HaloVZ[ihalo],HaloR[ihalo]);
			...
			check = check_HaloR_in_mesh(ihalo,HaloX,HaloY,HaloZ,HaloR,i,j,k,ListOfHalos,NHalosPerCellStart,NHalosPerCellEnd);
			#endif
//			if (ihalo<=-1)
...
//		if (ihalo%500==0)
//			fprintf(stderr,"\t cell = %ld =[%ld,%ld,%ld]. part %ld. #haloincell %ld \n",lin_ijk,i,j,k,ipart,(NHalosPerCellEnd[lin_ijk]-NHalosPerCellStart[lin_ijk]));
//		if (ihalo<=-1)
...
long select_part(long ijk,long **ListOfPart,long *NPartPerCell,float *dens){
	long i_rnd,ipart;
...
long select_part(long ijk,long **ListOfPart,long *NPartPerCell,float *dens){
	...
	for (i=0;i<NPartPerCell[ijk];i++){
		Prob += pow((double)dens[ListOfPart[ijk][i]],(double)BETA);
	}
	d_rnd = ((float)rand()/(RAND_MAX+1.0))*Prob;
	Prob=0.;
	for (i=0;i<NPartPerCell[ijk];i++){
...
//returns 1 if there is any collision, returns 0 for no collision
int check_HaloR_in_mesh(long ihalo,float *X, float *Y, float *Z , float *R,long i,long j,long k,long *ListOfHalos,long *NHalosPerCellStart, long *NHalosPerCellEnd){
	int l,m,n;
	for (l=i-1;l<=i+1;l++){
	for (m=j-1;m<=j+1;m++){
	for (n=k-1;n<=k+1;n++){
